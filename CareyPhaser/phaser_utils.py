...
def upsample( t, y, mul=2 ):
  """Interpolate an array uniformly in time.

  INPUT:
    t -- N -- sequence of times; must be monotone increasing
    y -- N x W... -- data array with first index of length N

  OUTPUT: t0, y0
    t0 -- N*mul -- sample times (uniform rate)
    y0 -- (N*mul) x W... -- Interpolated data array
  """
  y = asarray(y)
  N = len(t)
  if y.shape[0] != N:
    raise IndexError("y.shape[0]==%d must equal len(t)==%d" % (y.shape[0],N))
  sz = y.shape[1:]
  W = y.size / N
  y = y.reshape([N,y.size/N])
  t0 = linspace(t[0],t[-1],N*mul)
  y0 = concatenate([
      interp(t0,t,y[:,k])[:,newaxis]
      for k in xrange(W)
  ],axis=1)
  return t0, y0.reshape((len(t0),)+tuple(sz))
